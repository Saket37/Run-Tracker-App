...
class ActiveRunViewModel(
    ...
    private val shouldTrack =
        snapshotFlow { state.shouldTrack }.stateIn(viewModelScope, SharingStarted.Lazily, state)

    private val isTracking = combine(
        shouldTrack, hasLocationPermission
    ) { shouldTrack, hasLocationPermission ->
        shouldTrack as Boolean && hasLocationPermission
    ...
    fun onAction(action: ActiveRunAction) {
        when (action) {
            ...
            ActiveRunAction.OnToggleRunClick -> {
                state = state.copy(
                    hasStartedRunning = true, shouldTrack = !state.shouldTrack
                )
            }
    ...
    private fun finishRun(mapPictureBytes: ByteArray) {
        ...
        viewModelScope.launch {
            val run = Run(
                ...
                dateTimeUTC = ZonedDateTime.now().withZoneSameLocal(ZoneId.of("UTC")),
                distanceInMeters = state.runData.distanceMeters,
                location = state.currentLocation ?: Location(0.0, 0.0),
                maxSpeedKmH = LocationDataCalculator.getMaxSpeed(locations),
                totalElevationInMeters = LocationDataCalculator.getTotalElevationMeters(locations),
                mapPictureUrl = null
            )
